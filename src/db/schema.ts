...
export const retreats = pgTable(
  'retreats',
  {
    ...
    // Lightweight CRM archive flag. Archived retreats stay queryable via
    // the Archived dashboard tab but disappear from the active list.
    archivedAt: timestamp('archived_at', { withTimezone: true }),
    archivedByTherapistId: uuid('archived_by_therapist_id').references(
      () => therapists.id,
      { onDelete: 'set null' },
    ),
    ...
    /**
     * One-shot idempotency key from /admin/clients/new. Prevents a retried
     * POST from creating duplicate retreats and sending duplicate consent
     * package emails.
     */
  ...
  (t) => ({
    ...
    createRequestIdIdx: uniqueIndex('retreats_create_request_id_idx')
      .on(t.createRequestId)
      .where(sql`${t.createRequestId} IS NOT NULL`),
    ...
    archivedIdx: index('retreats_archived_idx').on(t.archivedAt),
...
/**
 * Public contact inquiries. These are pre-client records: a visitor has
 * asked to speak with one therapist, but no client/retreat row exists yet.
 * Every identity and message field is PHI-bearing and lives only in the
 * ITR PHI project at runtime.
 */
export const contactInquiries = pgTable(
  'contact_inquiries',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    requestedTherapistId: uuid('requested_therapist_id').references(
      () => therapists.id,
      { onDelete: 'set null' },
    ),
    assignedTherapistId: uuid('assigned_therapist_id').references(
      () => therapists.id,
      { onDelete: 'set null' },
    ),
    status: inquiryStatus('status').notNull().default('new'),

    // PHI: name + contact + location.
    firstName: text('first_name').notNull(),
    lastName: text('last_name').notNull(),
    email: text('email').notNull(),
    ...
    consultationWindows: jsonb('consultation_windows')
      .notNull()
      .default(sql`'[]'::jsonb`),
    ...
    consentText: boolean('consent_text').notNull().default(false),
    consentEmail: boolean('consent_email').notNull().default(false),
    policyServiceLevel: boolean('policy_service_level').notNull().default(false),
    policyFinancial: boolean('policy_financial').notNull().default(false),
    ...
    convertedRetreatId: uuid('converted_retreat_id').references(
      () => retreats.id,
      { onDelete: 'set null' },
    ),
    ...
    archivedAt: timestamp('archived_at', { withTimezone: true }),
    lastActionByTherapistId: uuid('last_action_by_therapist_id').references(
      () => therapists.id,
      { onDelete: 'set null' },
    ),
    createdAt: timestamp('created_at', { withTimezone: true })
  ...
  (t) => ({
    assignedStatusCreatedIdx: index('contact_inquiries_assigned_status_created_idx').on(
      t.assignedTherapistId,
      t.status,
      t.createdAt,
    ),
    contactHashCreatedIdx: index('contact_inquiries_contact_hash_created_idx').on(
      t.contactHash,
      t.createdAt,
    ),
    messageHashCreatedIdx: index('contact_inquiries_message_hash_created_idx').on(
      t.messageHash,
      t.createdAt,
    ),
    convertedRetreatIdx: index('contact_inquiries_converted_retreat_idx').on(
      t.convertedRetreatId,
    ),
  }),
);

export const contactInquiryEvents = pgTable(
  'contact_inquiry_events',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    inquiryId: uuid('inquiry_id')
      .notNull()
      .references(() => contactInquiries.id, { onDelete: 'cascade' }),
    actorTherapistId: uuid('actor_therapist_id').references(() => therapists.id, {
      onDelete: 'set null',
    }),
    eventType: text('event_type').notNull(),
    payload: jsonb('payload').notNull().default(sql`'{}'::jsonb`),
  ...
  (t) => ({
    inquiryCreatedIdx: index('contact_inquiry_events_inquiry_created_idx').on(
      t.inquiryId,
      t.createdAt,
    ),
  }),
...
export type ContactInquiryEvent = typeof contactInquiryEvents.$inferSelect;
